...
public class RoundKnobButton extends RelativeLayout implements OnGestureListener {
	...
	public RoundKnobButton(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	public void RoundKnobButton2(Context context, int rotoron, final int w, final int h) {
		// we won't wait for our size to be calculated, we'll just store out fixed size
		m_nWidth = w; 
		m_nHeight = h;

		Bitmap srcon = BitmapFactory.decodeResource(context.getResources(),  R.drawable.wheel);
	...
	public void RoundKnobButton3(Context context, int rotoron, final int w, final int h) {
		// we won't wait for our size to be calculated, we'll just store out fixed size
		m_nWidth = w;
		m_nHeight = h;
	...
	public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
		...
		if (! Float.isNaN(rotDegrees)) {
			...
			if (posDegrees > 210 || posDegrees < 150) {
				...
				int percent = (int) (scaleDegrees / 20);
				if (m_listener != null)
